...
- Added an anti-spam function which filters messages posted by the same author within less than 3 seconds of each other.
- Added the version command. This just prints the version of the bot.
- Fixed the mentions on the \"hello\" \"goodbye\" and member welcome commands.

The spam filter is a work-in-progress feature which will require a testing period. Please, do provide feedback.
```""".format(version)
...
async def on_message(message):
    ...
    msg_time_second = msg_time.second
    
    spam_limit = msg_time_second - 3
    
    msg = message.content
    channel = message.channel
    words = msg.split()
    
    print(history.get(str(message.author)))
    if history.get(str(message.author)) <= spam_limit:
        print("SPAM")
    else:
        print("Not spam")
    history[str(message.author)] = msg_time_second
